...
if newImport:
    ...
    t_stat = [45, 60] # Static Movement
    n_stat = [np.where(t >= t_stat[0])[0][0], np.where(t >= t_stat[1])[0][0]]
    t_stat = t[n_stat[0]:n_stat[1]+1]
    ...
    t_temp = t_stat[N:-N]
    for ii in range(1,N):
        t_temp = t_temp + t_stat[N+ii:-N+ii] + t_stat[N-ii:-N-ii]
    ...
    pos_stat = pos[n_stat[0]: n_stat[1]+1, :]

    N_link = 7
    posEE_stat = np.zeros((pos_stat.shape[0], 3)) # Sum up all links -- end effector position

    for ii in range(3):
        for jj in range(N_link):
            posEE_stat[:,ii] = posEE_stat[:,ii] + pos_stat[:,ii+jj*3]

            
    
    
    t_range = [900, 1000] # Regular movement
...
ax3.plot(posEE[:,0], posEE[:,1], posEE[:,2], '-', color=[0,0,0.6], label='Dynamic trajectory')
ax3.plot(posEE_stat[:,0], posEE_stat[:,1], posEE_stat[:,2], '-', color=[0.7,0,0.0], label='Statict trajectory', linewidth=2)
